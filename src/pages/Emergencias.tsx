...
// 🔴 FIX AUDITORÍA: Usamos el hook oficial tipado en lugar del casteo inseguro
import { useMunicipality } from "@/context/MunicipalityContext";

// Interface estricta para evitar 'any'
interface DBSettingsPublic {
  emergency_phone: string | null;
}

export default function Emergencias() {
  // 🔴 FIX AUDITORÍA: Consumo seguro y desestructurado
  const { municipalityId, municipality: municipalityName } = useMunicipality();
    ...
    const loadPhone = async () => {
      ...
      try {
        ...
        if (!error && isMounted && data) {
          const rawData = data as unknown as DBSettingsPublic;
          setMuniPhone(rawData?.emergency_phone || null);
        }
      } catch (err) {
  ...
  const items = muniPhone
    ? [...baseItems, { label: `Municipalidad de ${municipalityName || 'tu municipio'}`, number: muniPhone, icon: Building2, hint: "Atención al vecino" }]
    : baseItems;
  ...
  return (
    <div className="space-y-4">
      <header>
        ...
        <p className="text-sm text-muted-foreground">Llamada directa, 24 horas. {municipalityName || 'tu municipio'}.</p>
      </header>
        ...
        <div className="space-y-3">
          {items.map((it) => (
            <a
              key={it.label}
              href={`tel:${it.number}`}
              className="flex items-center gap-4 p-4 rounded-[16px] min-h-[64px] hover:bg-red-200 transition-colors"
