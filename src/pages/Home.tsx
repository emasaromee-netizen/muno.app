...
const Tile = ({ to, icon: Icon, title, sub, color, onClick }: TileProps) => {
  ...
  return (
    <Link
      to={to}
      onClick={handleClick}
      className="isa-card p-4 hover:-translate-y-0.5 hover:shadow-md transition-all flex flex-col gap-2"
    >
      <div
      ...
      <div>
        <div className="text-[15px] font-extrabold text-isa-navy leading-tight">
          {title}
        </div>
...
const Metric = ({ label, value, hint }: MetricProps) => (
  <div className="isa-card p-4">
    ...
    <div className="text-2xl font-extrabold text-isa-navy mt-0.5">
      {value}
    </div>
      ...
      Intendencia: [
        {
          ...
          label: "Banners Home",
          icon: Megaphone,
          color: "hsl(var(--muno-emerald))",
          sub: "Avisos y novedades",
        },
        {
          to: "/admin/metricas",
          label: "Métricas Turísticas",
          icon: BarChart3,
