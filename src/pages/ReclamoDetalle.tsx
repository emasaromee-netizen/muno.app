...
// 1. Interfaz Estricta para erradicar los 'any'
interface ClaimData {
  id: string;
  status: string;
  category: string;
...
export default function ReclamoDetalle() {
  ...
  // 2. Efecto blindado contra fugas de memoria
  useEffect(() => {
    let isMounted = true;
    ...
    const fetchClaim = async () => {
      try {
        ...
        if (isMounted && data) {
          const c = data as unknown as ClaimData;
          setClaim(c);
          setRated(c.rating || null);
        }
      } catch (err) {
        console.error("Error cargando el detalle del reclamo:", err);
      } finally {
  ...
  if (loading) {
    return (
      <div className="text-center py-12 inline-flex items-center gap-2 justify-center w-full">
  ...
  if (!claim) {
    return (
      <div className="text-center py-12">
        Reclamo no encontrado. <Link to="/" className="text-muno-blue underline">Volver</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <Link to="/mi-cuenta" className="inline-flex items-center gap-1 text-sm font-bold text-isa-navy">
        <ArrowLeft className="w-4 h-4" /> Mis incidentes
      </Link>
      ...
      <div className="isa-card p-5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className={`isa-chip ${STATUS_STYLE[claim.status] || ""}`}>
            <Icon className="w-3 h-3" />{STATUS_LABEL[claim.status] || claim.status}
          </span>
        </div>
        <div className="text-sm">
          <span className="text-muted-foreground">Categoría: </span>
          <span className="font-bold">{claim.category}</span>
        </div>
        <div className="text-sm">
          <span className="text-muted-foreground">Fecha: </span>
          <span className="font-bold">{new Date(claim.created_at).toLocaleString("es-AR")}</span>
        </div>
        {claim.address && (
          <div className="text-sm">
